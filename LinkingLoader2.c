...
int Write_To_File(char byte[2],int address);
int flag=0 , count = 0;
...
void main(){
   ...
   char text_record_length[6],byte[2],m_address[6],m_name[10],half_length[2],op;
    ...
    obj_file = fopen("object_file.txt","r");
    lk_op = fopen("linked_output.txt","a");
        ...
        if(record[0] == 'H'){
            ...
            if(previous_address < addr){
                ...
                while(previous_address < addr){
                    strncpy(byte,"..",2);
                    Write_To_File(byte,previous_address);
                    /*if(flag == 0){
...
int Write_To_File(char byte[2],int addr){
    FILE *lk_op;
    lk_op = fopen("linked_output.txt","a");
    if(flag == 0){
        fprintf(lk_op,"%x\t",addr);
